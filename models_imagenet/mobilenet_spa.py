...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.spa = SPALayer(oup)

    def forward(self, x):

        if self.use_res_connect:
            return x + self.spa(self.conv(x))
        else:
            return self.spa(self.conv(x))
...
def spa_mobilenet_v2(pretrained=False, **kwargs):
    """
...
def demo():
    net = spa_mobilenet_v2(num_classes=1000)
